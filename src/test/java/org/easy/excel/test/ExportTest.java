...
public class ExportTest {
	 ...
	 */
	@Test
	public void testExportUseTemplate()throws Exception{
		Workbook workbook = ExcelUtil.getWorkBookByPath("classpath:exampleTemplate.xlsx");
		//TODO 如果模板只有第一页的提示语，则不需要删除，根据实际情况定
		//模板是随便找的不需要过多关注里面的内容
		workbook.removeSheetAt(1);//删除索引位1的sheet页
		Workbook wk = context.createExcel(excelId, getStudents(), null, null, workbook);
		wk.write(new FileOutputStream("src/test/resources/test-export-use-template-excel.xlsx"));
	}
