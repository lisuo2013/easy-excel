...
import org.easy.excel.parsing.DefaultCellValueConverter;
	...
	public Object convert(Object bean, Object value, FieldValue fieldValue, Type type, int rowNum)
		...
		if(type==Type.IMPORT){
			if(queryForDb(val.toString())){
				...
				throw new ExcelDataException("在数据库中没有找到["+value.toString()+"]的用户信息", rowNum, fieldValue.getAlias(),value.toString(),bean);
			}
		}else{
